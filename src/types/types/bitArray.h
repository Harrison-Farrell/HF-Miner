...
protected:
    ...
    static_assert(BITS % 8 == 0, "BitArray can only store bytes. BITS must be a multiple of 8");
    std::array<uint8_t, mBytes> mdata;
    static_assert(mBytes == sizeof(mdata), "BitArray must have at least one byte");
...
public:
    ...
    constexpr explicit BitArray(std::span<const unsigned char> char_bytes) {
        ...
        assert(char_bytes.size() == mBytes && "Input byte span size must match BitArray size");
            std::copy(char_bytes.begin(), char_bytes.end(), mdata.begin());
    }   

    /// \brief Constructor from a hexadecimal string.
    /// \param hex_str Hexadecimal string representation of bits to initialize the BitArray.
    ...
    constexpr int Compare(const BitArray& other) const { return std::memcmp(mdata.data(), other.mdata.data(), mBytes); }

    /// \brief Check if all bits are zero.
    /// \return true if all bits are zero, false otherwise.
    constexpr bool IsNull() const
    {
        return std::all_of(mdata.begin(), mdata.end(), [](uint8_t val) {
            return val == 0;
        });
    }
    ...
    constexpr uint64_t GetUint64(int pos) const { return ReadLE64(mdata.data() + pos * 8); }
...
{
    if (hex_str.length() != mdata.size() * 2) throw "Hex string must fit exactly";
